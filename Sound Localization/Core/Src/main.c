...
/**
  ******************************************************************************
  * @file    main.c
  * @brief   Startup: periferije → I2C/IMU init → taskovi → scheduler
  ******************************************************************************
  */
...
#include "adc_driver.h"      /* Custom_ACQ_Start */
#include "task_manager.h"    /* app_tasks_init */
#include "app_init.h"        /* app_init (I2C + IMU + dijagnostika) */
...
/* USER CODE BEGIN PV */
/* RTOS taskovi i queue-ovi kreiraju se u task_manager.c (app_tasks_init).
 * I2C se inicijalizira LL driverom (i2c_driver.c), ne HAL handle-om. */
/* USER CODE END PV */
...
/* USER CODE BEGIN PFP */
/* I2C1 inicijalizira LL driver (Custom_I2C1_Init iz i2c_driver.c), ne CubeMX —
 * zato ovdje nema MX_I2C1_Init. Taskovi (Start*Task) su u task_manager.c. */
...
int main(void)
{
  ...
  MX_TIM8_Init();
  /* USER CODE BEGIN 2 */
  /* Aplikacijski init (I2C LL + IMU + dijagnostika + kalibracija žira). Lokalizacija
   * (time-domain, LOC_INIT) i RTOS objekti su u task_manageru (app_tasks_init). */
  app_init();
  /* USER CODE END 2 */
  ...
  /* Svi queue-ovi i taskovi (ACQ/LOC/UART/IMU) kreiraju se u task_manager.c.
...
/* USER CODE BEGIN 4 */
/* HAL_TIM_PeriodElapsedCallback (TIM6 → HAL_IncTick) generira CubeMX niže. */
...
/* Taskovi (StartACQTask / StartLOCTask / StartUARTTask / StartIMUTask) i njihove
 * implementacije su u task_manager.c. defaultTask se ne koristi. */
