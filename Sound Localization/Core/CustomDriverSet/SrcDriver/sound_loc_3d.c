...
/*
 * sound_loc_3d.c — puna 4-mikrofonska 3D lokalizacija. Vidi sound_loc_3d.h za
 * opis algoritma i koordinatni sustav.
 *
 * Debug snapshot varijable (dbg_*) postavljaju se pri svakoj detekciji i čitaju
 * se u debuggeru; ne idu na UART.
 */
...
/* Detekcijski pragovi (slabiji / neujednačeni mikrofoni).
...
/* Broj LOC3D_Process poziva koji se ignoriraju nakon svake detekcije (cooldown).
 * Frame = half-buffer = 1024/192kHz ≈ 5.33 ms. 45 × 5.33 ms ≈ 240 ms — propusti
 * rep pljeska i prve odjeke. */
 ...
 * Računa se u LOC3D_Init() iz MIC*_{X,Y,Z} (audio_common.h).
 */
 ...
 * unutar 2×SAMPLES_PER_CHANNEL buffera, gledajući energiju sva 4 kanala.
 */
...
uint8_t LOC3D_Process(const uint16_t *buf, loc3d_result_t *out)
{
    ...
    /* 4. GCC-PHAT za tri para (M1 = referentni). */
     ...
     *   M2 = RANK2 → (j−1)=1 ; M3 = RANK3 → (j−1)=2 ; M4 = RANK4 → (j−1)=3 */
    ...
    /* 6. Geometrijsko rješenje — u_propagacije = M_geom · tau ; smjer PREMA
     * izvoru s = −u, normaliziran. */
    ...
    /* 7. Kutovi.
     * Azimut: atan2 ∈ (-180,180] → wrap u [0,360).
     * Elevacija: asin(sz) ∈ [-90,+90] (fizički pun raspon, ne wrapa se). */
