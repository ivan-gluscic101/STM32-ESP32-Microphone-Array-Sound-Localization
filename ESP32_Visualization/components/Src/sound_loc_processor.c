 ...
 *                           NAPOMENA: YW i FLAGS se na ESP32 NE koriste — yaw
 *                           se preskače, vizualizacija ide samo po roll/pitch.
 *
...
static void rx_task(void *arg) {
    ...
    /* Orijentacija (Type 0x05) — koristimo samo roll i pitch */
    ...
    while (1) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            switch (state) {
                ...
                case S_ORI_YW_H: state = S_ORI_YW_L;  break;  // yaw se ignorira
                case S_ORI_YW_L: state = S_ORI_FLAGS; break;  // (samo poravnanje okvira)
                case S_ORI_FLAGS: state = S_EOF1; break;      // FLAGS se ne koristi
                case S_EOF1:
                ...
                case S_EOF2:
                    if (b == ANGLE_PKT_EOF2) {
                        if (pkt_type == ANGLE_PKT_TYPE_RAW) {
                             ...
                             * Yaw i FLAGS se ne koriste (samo roll/pitch). */
